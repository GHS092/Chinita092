...
RUN git clone --depth 1 https://github.com/GHS092/HERM.git /opt/hermes-agent && \
    cd /opt/hermes-agent && \
    uv pip install --system --no-cache -e ".[messaging,honcho,mcp,computer-use,pty,cli]" && \
    cd /opt/hermes-agent/web && \
    npm install --silent && \
    npm run build && \
    cd /opt/hermes-agent/scripts/whatsapp-bridge && npm install --omit=dev --silent && \
...
COPY fetch_transcript.py /app/fetch_transcript.py

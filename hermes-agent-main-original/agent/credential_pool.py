...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "openrouter":
        ...
        tokens_str = _get_env_prefer_dotenv("OPENROUTER_CAROUSEL_KEYS") or _get_env_prefer_dotenv("OPENROUTER_API_KEY")
        if tokens_str:
            tokens = [t.strip() for t in tokens_str.split(",") if t.strip()]
            for i, token in enumerate(tokens):
                source = f"env:OPENROUTER_API_KEY_{i}" if i > 0 else "env:OPENROUTER_API_KEY"
                ...
                changed |= _upsert_entry(
                    ...
                    source,
                    {
                        ...
                        "label": f"OPENROUTER_API_KEY_{i}" if i > 0 else "OPENROUTER_API_KEY",
                    },
                )
        return changed, active_sources
    ...
    for env_var in env_vars:
        # Prefer ~/.hermes/.env over os.environ
        carousel_var = env_var.replace("_API_KEY", "_CAROUSEL_KEYS")
        tokens_str = _get_env_prefer_dotenv(carousel_var) or _get_env_prefer_dotenv(env_var)
        if not tokens_str:
            continue
        tokens = [t.strip() for t in tokens_str.split(",") if t.strip()]
        for i, token in enumerate(tokens):
            source = f"env:{env_var}_{i}" if i > 0 else f"env:{env_var}"
            if _is_source_suppressed(provider, source):
                continue
            active_sources.add(source)
            ...
            changed |= _upsert_entry(
                ...
                source,
                {
                    ...
                    "access_token": token,
                    "base_url": base_url,
                    "label": f"{env_var}_{i}" if i > 0 else env_var,
                },
            )
    return changed, active_sources

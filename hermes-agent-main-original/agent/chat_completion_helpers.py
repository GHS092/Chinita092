...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    # Force clamp max_tokens to 4096 to prevent 402/quota errors (from server.py migration)
    if getattr(agent, "max_tokens", None) is None or getattr(agent, "max_tokens", 0) > 4096:
        agent.max_tokens = 4096
    _ephemeral_out = getattr(agent, "_ephemeral_max_output_tokens", None)
    if _ephemeral_out is not None and _ephemeral_out > 4096:
        agent._ephemeral_max_output_tokens = 4096
